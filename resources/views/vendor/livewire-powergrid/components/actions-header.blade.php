...
<button wire:click='$emit("{{ $action->event }}", @json($parameters))' title="{{ $action->tooltip }}"
...
@elseif($action->view !== '')
<button wire:click='$emit("openModal", "{{$action->view}}", @json($parameters))' title="{{ $action->tooltip }}"
    class="{{ filled($action->class) ? $action->class : $theme->actions->headerBtnClass }}">
    {!! $action->caption !!}
</button>
@else
...
<form target="{{ $action->target }}" action="{{ route($action->route, $parameters) }}" method="{{ $action->method }}">
    ...
    @csrf
    <button type="submit" title="{{ $action->tooltip }}"
        class="{{ filled( $action->class) ? $action->class : $theme->actions->headerBtnClass }}">
        {!! $action->caption ?? '' !!}
    </button>
</form>
...
<a href="{{ route($action->route, $parameters) }}" title="{{ $action->tooltip }}" target="{{ $action->target }}"
    class="{{ filled($action->class) ? $action->class : $theme->actions->headerBtnClass }}">
    {!! $action->caption !!}
</a>
@endif
